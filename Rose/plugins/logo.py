...
from . import , OWNER_NAME, download_file, get_string, mediainfo, ultroid_cmd
...
async def logo_gen(event):
    ...
    if not bg_:
        if event.client._bot:
            ...
            async for i in event.client.iter_messages(
                "@GangOfMafiaAssistant", filter=InputMessagesFilterPhotos
            ):
                pics.append(i)
    ...
    if os.path.exists(flnme):
        await event.client.send_file(
            event.chat_id,
            file=flnme,
            caption=f"Logo by [{BOT_NAME}](tg://user?id={BOT_USERNAME})",
            force_document=True,
        )
        os.remove(flnme)
        await xx.delete()
